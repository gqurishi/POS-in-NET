    ...
    private async void OnSyncOrdersClicked(object sender, EventArgs e)
    {
        try
        {
            ...
            for (int daysAgo = 0; daysAgo < 7; daysAgo++)
            {
                ...
                {
                    System.Diagnostics.Debug.WriteLine($"📦 {targetDate:MMM dd}: {syncResult.Message}");
                    daysWithOrders++;
                }
                totalOrders += syncResult.OrdersFound;
            }
            
            System.Diagnostics.Debug.WriteLine($"✅ Quick sync complete: {totalOrders} orders from {daysWithOrders} days");
    ...
    private async void OnSyncHistoricalClicked(object? sender, EventArgs e)
    {
        ...
        try
        {
            ...
            for (int daysAgo = 0; daysAgo < 60; daysAgo++)
            {
                ...
                if (syncResult.OrdersFound > 0)
                {
                    System.Diagnostics.Debug.WriteLine($"📦 {targetDate:MMM dd, yyyy}: {syncResult.Message}");
                    daysWithOrders++;
                ...
                if ((daysAgo + 1) % 5 == 0)
                {
                    ...
                    SyncHistoricalButton.Text = $"⏳ {progress}% - {totalOrders} orders found";
                    System.Diagnostics.Debug.WriteLine($"⏳ Progress: {daysAgo + 1}/60 days ({progress}%), {totalOrders} orders, {daysWithOrders} days with data");
                }
            }
            
            System.Diagnostics.Debug.WriteLine("========================================");
            System.Diagnostics.Debug.WriteLine($"✅ HISTORICAL SYNC COMPLETE");
            System.Diagnostics.Debug.WriteLine($"📊 Total: {totalOrders} orders from {daysWithOrders} days");
            System.Diagnostics.Debug.WriteLine($"🆕 New orders: {newOrders}");
